...
best_val = 0.0
last_improvement = 0
model_filename = "MNIST-" * string(pm) * "-l" * string(lambda) * "-e" * string(eta) * ".bson"
...
val_tr = evaluate(giant_train_set...)
@info(@sprintf("[%s]: Test metric: %.4f, Train metric: %.4f", "MNIST", val_ts, val_tr))
...
if pm.info.n_constraints > 0
    ...
    @info(@sprintf("[%s]: Test cs: %.4f, Train cs: %.4f", "MNIST", cs_ts[1], cs_tr[1]))
end
