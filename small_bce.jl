...
best_acc = 0.0
last_improvement = 0
model_filename = "BCE" * "-" * dname * "-l" * string(lambda) * "-e" * string(eta) * ".bson"
for epoch_idx in 1:100
    ...
    acc_tr = accuracy(X_tr |> gpu, y_tr |> gpu)
    @info(@sprintf("[%d]: Validation accuracy: %.4f, Train accuracy: %.4f", epoch_idx, acc, acc_tr))

    # If this is the best accuracy we've seen so far, save the model out
    if acc >= best_acc
        @info(" -> New best accuracy! Saving model out to " * model_filename)
        model_cpu = model |> cpu
