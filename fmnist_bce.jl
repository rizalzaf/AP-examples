...
function run(lambda::Real = 0.0)
    ...
    log_filename = "log/BCE" * "-FashionMNIST" * ".log"
    ...
    # objective(x, y) = ap_objective(model(x), y, pm) + lambda * sum(x -> sum(x .* x), params(model))  # l2 reg
...
function main(args)
    ...
    # save best models
    model_filename = "BCE" * "-" * "FashionMNIST" *  ".bson"
